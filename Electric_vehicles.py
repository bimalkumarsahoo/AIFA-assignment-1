...
def main():
    ...
    cars_cols = ['Cars ID', 'Source', 'destination', 'battery_status', 'charging_rate',
                 'discharge_rate', 'Max_battery', 'avg_speed']
    cars_data, distance_data = data_input(path1, path2, cars_cols)
    ...
    for idx in all_cars.keys():
        ...
        total.append(sum(dict(all_cars[idx].min_time).values()))

    print("\nTotal time taken. (Maximum time taken among all the cars):  ", max(total))
